    ...
    for item in os.listdir("."):
        if os.path.isdir(item) and item != "." and item != ".." and not item.startswith("."):
            ...
            if len(os.listdir(item)) > 0:
                 ...
                 if zips:
                     try:
                         with zipfile.ZipFile(os.path.join(item, zips[0]), 'r') as zf:
                             # search for addon.xml at root level inside first folder
                             # we also look for icon.png, fanart.jpg, etc.
                             
                             # We need to find the root folder inside the zip first
                             # e.g. "plugin.video.foo/"
                             root_folder = None
                             for name in zf.namelist():
                                 if name.endswith('addon.xml') and name.count('/') == 1:
                                     # This logic assumes structure is root_folder/addon.xml
                         ...
                         print(f"Error extracting from {zips[0]}: {e}")
