...
def download_release(repo_url):
    try:
        ...
        # Handle tags like 'vfs-12.3.7.1' or 'v1.0.0'
        # Extract version using regex to find first numeric sequence
        version_match = re.search(r'(\d+(\.\d+)+[a-z0-9\-]*)', tag_name)
        if version_match:
            version = version_match.group(1)
            # Remove trailing non-version characters if any, but be careful
            # Kodi versions can have suffixes like -beta1
        else:
